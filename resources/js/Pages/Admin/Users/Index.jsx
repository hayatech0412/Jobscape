...
export default function Index({
    ...
    const deleteUser = () => {
        deleteForm.post(route('admin.users.delete', [selectedUser.id]), {
            onFinish: () => {
        ...
        <AdminAuthLayout>
            <div className="w-[92%] mx-auto mt-[24px] pb-[50px] bg-white border border-gray-200 rounded-lg shadow-md">
                <div className="border-b border-gray-200 px-[4%] py-4">
                    <h3 className=" text-lg font-semibold text-gray-600">ユーザー一覧</h3>
                </div>
                            ...
                            <tbody>
                                {users.data.map((user, index) => (
                                    <tr key={user.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="border px-4 py-2">{ user.profile.last_name + ' ' + user.profile.first_name }</td>
                                        <td className="border px-4 py-2">{user.email}</td>                                        
                                        <td className="border px-4 py-2">{user.profile.phone_number}</td>
                                        <td className="border px-4 py-2">{user.profile.nickname}</td>
                                        <td className="border px-4 py-2">
                                            <Link href={route('admin.users.show', [user.id])} className="text-blue-500 hover:text-blue-700">詳細</Link>
                                            <Link href={route('admin.users.edit', [user.id])} className="ml-2 text-cyan-500 hover:text-blue-700">編集</Link>
                                            <button onClick={() => toggleModal((users.current_page - 1) * users.per_page + index + 1, user) } className="ml-2 text-red-500 hover:text-blue-700">削除</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            <DeleteModal data={isDeleteSelected} isOpen={isOpenDeleteModal} onClose={toggleModal} onSubmit={deleteUser} />
        </AdminAuthLayout>
    );
}
