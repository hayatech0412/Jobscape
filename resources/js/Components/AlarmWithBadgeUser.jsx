...
export default function AlarmWithBadgeUser({ 
    notify 
}) {
    ...
    const toggleMenu = () => {
        if (!isOpen) {
            axios.get(route("notifications.open")).then((response) => {
                // setIsConfirm(
    ...
    return (
        <button className="ml-4 relative" onClick={toggleMenu}>
            ...
            <ul
                ref={menuRef}
                className={`absolute top-8 right-0 w-[250px] bg-white shadow-[0_0_10px_0_rgba(0,0,0,0.1)] rounded-lg py-4 z-50 ${
                    isOpen ? "block" : "hidden"
                }`}
            >
