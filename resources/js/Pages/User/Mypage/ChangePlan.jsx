...
export default function ChangePlan({
    ...
    const submit = (e, plan_id) => {
        ...
        post(route('payments.plan.change'), {
            onFinish: () => {},
        });
    };
                    ...
                    <div className="grid gap-0 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 sm:gap-2">
                        {plans.map((plan) => (
                            ...
                            >
                                ...
                                {plan.id === profile.plan_id ? (
                                    <div className="bg-[#ff5256] absolute top-2 left-2 w-[100px] h-[40px] rounded-full flex justify-center items-center text-white text-center text-[13px] mb-3">
                                        現在のプラン
                                    </div>
                                ) : ''}
