...
export default function FilterButton({
    ...
    return (
        <div className='relative flex flex-col justify-center items-center space-x-2 cursor-pointer' onClick={toggleMenu}>
            <span className="flex flex-col items-center justify-center w-[52px] text-[12px] gap-1 leading-none">
                <FilterButtonIcon className="size-4" color='#d1d1d1' />
                {options.find(x => x.value == value)?.label ?? options[0].label}
            </span>
            <ul ref={menuRef} className={`z-20 absolute top-10 right-0 w-[150px] bg-white text-[14px] shadow-[0_0_10px_0_rgba(0,0,0,0.1)] rounded-lg py-4 ${isOpen ? 'block' : 'hidden'}`}>
